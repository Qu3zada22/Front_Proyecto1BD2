...
export default function PropietarioDashboard() {
  ...
  const myRestaurantes = restaurantes.filter(
  ...
  const todayOrders = myOrders.filter(
    (o) =>
      new Date(o.fecha_creacion).toDateString() === new Date().toDateString(),
  );
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button asChild>
          <Link to="/propietario/nuevo-restaurante">
      ...
      {myRestaurantes.length === 0 ? (
        <div className="py-12 text-center">
          <p className="text-lg text-muted-foreground">
            Aun no tienes restaurantes registrados
          </p>
          <Button asChild className="mt-4">
            <Link to="/propietario/nuevo-restaurante">
              <Plus size={16} /> Agregar tu primer restaurante
            </Link>
          </Button>
        </div>
      ) : (
              ...
              <CardContent className="p-4">
                ...
                <div className="mt-1 flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {r.calificacion_prom.toFixed(1)} ({r.total_resenas})
                  </span>
                </div>
