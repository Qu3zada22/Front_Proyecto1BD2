...
export default function AdminUsuariosPage() {
  const { adminUsers, ordenes, toggleUserActivo, deleteUser, deleteUsers } = useData()
  ...
  const handleDeleteOne = () => {
    ...
    setSelected((prev) => { const next = new Set(prev); next.delete(confirmDeleteId); return next })
  ...
  return (
    <div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left">
              ...
              <th className="pb-3 pr-4 font-medium text-muted-foreground text-center">Gasto</th>
              <th className="pb-3 pr-4 font-medium text-muted-foreground text-center">Estado</th>
              <th className="pb-3 font-medium text-muted-foreground text-center">Acciones</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((u) => {
              ...
              return (
                <tr key={u._id} className={`border-b last:border-0 ${selected.has(u._id) ? "bg-primary/5" : ""}`}>
                  ...
                  <td className="py-3 text-center">
                    <div className="flex items-center justify-center gap-1">
                      <Button
                        variant="ghost"
                        size="sm"
                        className={u.activo ? "text-destructive hover:text-destructive" : "text-emerald-600 hover:text-emerald-600"}
      ...
      <Dialog open={!!confirmDeleteId} onOpenChange={(open) => !open && setConfirmDeleteId(null)}>
        <DialogContent>
          ...
          <p className="text-muted-foreground">
            Esta acción eliminará permanentemente al usuario <strong>{adminUsers.find((u) => u._id === confirmDeleteId)?.nombre}</strong>.
          </p>
          <DialogFooter>
            <Button variant="outline" onClick={() => setConfirmDeleteId(null)}>Cancelar</Button>
            <Button variant="destructive" onClick={handleDeleteOne}>Eliminar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={confirmBulkDelete} onOpenChange={setConfirmBulkDelete}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-foreground">Eliminar {selected.size} usuarios</DialogTitle>
          </DialogHeader>
