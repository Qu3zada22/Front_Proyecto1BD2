...
export default function AdminReportesPage() {
  ...
  const topRestaurantes = useMemo(() => {
    ...
    resenas
      ...
      .forEach((r) => {
        if (!grouped[r.restaurante_id!]) grouped[r.restaurante_id!] = { sum: 0, count: 0 }
  ...
  const topPlatillos = useMemo(() => {
    const grouped: Record<string, { nombre: string; total_vendido: number; ingresos: number }> = {}
    ordenes
      .filter((o) => o.estado === "entregado")
      .forEach((o) => {
        o.items.forEach((item) => {
          if (!grouped[item.item_id]) grouped[item.item_id] = { nombre: item.nombre, total_vendido: 0, ingresos: 0 }
  ...
  const ingresosMes = useMemo(() => {
    ...
    ordenes
      ...
      .forEach((o) => {
        ...
        if (!grouped[key]) grouped[key] = { restaurante_id: o.restaurante_id, total: 0 }
    ...
    return Object.entries(grouped)
      .map(([key, data]) => {
        ...
        return { periodo, nombre: rest?.nombre || data.restaurante_id, total: data.total }
      })
  ...
  const topUsuarios = useMemo(() => {
    ...
    ordenes
      .filter((o) => o.estado === "entregado")
      .forEach((o) => { grouped[o.usuario_id] = (grouped[o.usuario_id] || 0) + o.total })
    return Object.entries(grouped)
      .map(([userId, gasto]) => {
        const u = adminUsers.find((u) => u._id === userId)
        return { nombre: u?.nombre || userId, email: u?.email || "", rol: u?.rol || "", gasto }
      })
  ...
  return (
    <div>
      <h1 className="mb-2 text-2xl font-bold text-foreground">Reportes Analíticos</h1>
      <p className="mb-8 text-sm text-muted-foreground">
        Resultados de las 4 aggregation pipelines complejas definidas en el backend.
      </p>
      ...
      <div className="flex flex-col gap-6">
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Pipeline: resenas → $group $avg → $match ≥5 reseñas → $sort → $limit 10 → $lookup restaurantes
            </CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <tr key={i} className="border-b last:border-0">
                      ...
                      <td className="py-3 pr-4 text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <span className="text-foreground font-medium">{r.promedio}</span>
                        </div>
                      </td>
                      <td className="py-3 text-center text-foreground">{r.total_resenas}</td>
                    </tr>
                  ))}
                  {topRestaurantes.length === 0 && (
                    <tr><td colSpan={5} className="py-6 text-center text-muted-foreground">Sin datos suficientes</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Pipeline: ordenes entregadas → $unwind items → $group $sum cantidad + ingresos → $sort → $limit 10
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left">
                    ...
                    <th className="pb-3 pr-4 font-medium text-muted-foreground text-center">Unidades Vendidas</th>
                ...
                <tbody>
                  {topPlatillos.map((p, i) => (
                    <tr key={i} className="border-b last:border-0">
                      ...
                      <td className="py-3 pr-4 text-center text-foreground">{p.total_vendido}</td>
                      <td className="py-3 text-right font-semibold text-primary">Q{p.ingresos.toFixed(2)}</td>
                    </tr>
                  ))}
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Pipeline: ordenes entregadas → $group {'{'} restaurante_id, $year, $month {'}'} → $sum $toDecimal → $sort → $lookup restaurantes
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {ingresosMes.map((row, i) => (
                    <tr key={i} className="border-b last:border-0">
                      ...
                      <td className="py-3 text-right font-semibold text-primary">Q{row.total.toFixed(2)}</td>
                    </tr>
                  ))}
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Pipeline: ordenes entregadas → $group $sum $toDecimal(total) → $sort → $limit 10 → $lookup usuarios (excluye password)
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {topUsuarios.map((u, i) => (
                    <tr key={i} className="border-b last:border-0">
                      ...
                      <td className="py-3 pr-4 text-center">
                        <Badge variant="outline" className="text-[10px] capitalize">{u.rol}</Badge>
                      </td>
                      <td className="py-3 text-right font-semibold text-primary">Q{u.gasto.toFixed(2)}</td>
                    </tr>
                  ))}
                  {topUsuarios.length === 0 && (
                    <tr><td colSpan={5} className="py-6 text-center text-muted-foreground">Sin datos</td></tr>
                  )}
                </tbody>
              </table>
            </div>
