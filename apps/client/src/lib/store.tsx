...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const loadOrdenes = useCallback(
    async (params?: { cliente_id?: string; restaurante_id?: string }) => {
      try {
  ...
  const loadOrdenesPropietario = useCallback(
    async (restauranteIds: string[]) => {
      if (!restauranteIds.length) return;
      try {
        const results = await Promise.all(
          restauranteIds.map((id) =>
            api.getOrders({ restaurante_id: id, limit: "100" }),
          ),
        );
        const all = results.flat() as Orden[];
        setOrdenes((prev) => {
          const others = prev.filter(
            (o) => !restauranteIds.includes(o.restaurante_id),
          );
          return [...others, ...all];
        });
      } catch (err) {
  ...
  const updateRestaurante = useCallback(
    async (id: string, data: Partial<Restaurante>) => {
      const updated = await api.updateRestaurante(id, data);
      setRestaurantes((prev) =>
        prev.map((r) => (r._id === id ? (updated as Restaurante) : r)),
      );
    },
    [],
  );
  ...
  const toggleRestauranteActivo = useCallback(
    (id: string) => {
      const rest = restaurantes.find((r) => r._id === id);
      if (!rest) return;
      api
        .updateRestaurante(id, { activo: !rest.activo })
        .then((updated) => {
          setRestaurantes((prev) =>
            prev.map((r) => (r._id === id ? (updated as Restaurante) : r)),
          );
  ...
  const updateMenuItem = useCallback(
    async (id: string, data: Partial<MenuItem>) => {
      ...
      setMenuItems((prev) =>
        prev.map((i) => (i._id === id ? (updated as MenuItem) : i)),
      );
    },
    [],
  );
  ...
  const toggleMenuItemDisponible = useCallback(
    (id: string) => {
      const item = menuItemsList.find((i) => i._id === id);
      if (!item) return;
      api
        .updateMenuItem(id, { disponible: !item.disponible })
        .then((updated) => {
          setMenuItems((prev) =>
            prev.map((i) => (i._id === id ? (updated as MenuItem) : i)),
          );
  ...
  const setMenuItemsDisponible = useCallback(
    (ids: string[], disponible: boolean) => {
      Promise.all(
        ids.map((id) => api.updateMenuItem(id, { disponible })),
      ).catch(console.error);
      setMenuItems((prev) =>
  ...
  const advanceOrderStatus = useCallback(
    async (orderId: string, _actorId: string) => {
      ...
      setOrdenes((prev) =>
        prev.map((o) => (o._id === orderId ? (updated as Orden) : o)),
      );
    },
    [ordenes],
  );
  ...
  const cancelOrder = useCallback(
    async (orderId: string, _actorId: string, _nota?: string) => {
  ...
  const addResena = useCallback(
    async (resena: Omit<Resena, "_id" | "fecha" | "activa" | "likes">) => {
      ...
      setRestaurantes((prev) =>
        prev.map((r) => {
          ...
          const avg =
            (r.calificacion_prom * r.total_resenas + resena.calificacion) / n;
          return {
            ...r,
