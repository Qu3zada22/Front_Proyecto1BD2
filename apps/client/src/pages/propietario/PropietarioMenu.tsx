...
export default function PropietarioMenu() {
  ...
  const addToPendingList = () => {
    ...
    setFormData({
      ...
      etiquetas: "",
      imagen: "",
      orden_display: items.length + pendingItems.length + 2,
    });
    setImgPreview("");
    setUploadedImageId(null);
  };
  ...
  const handleBulkDisponible = async (disponible: boolean) => {
    try {
      await Promise.all(
        [...selected].map((id) => api.updateMenuItem(id, { disponible })),
      );
      await fetchItems();
    } catch (err) {
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm text-foreground">
                Lista pendiente — {pendingItems.length}{" "}
                {pendingItems.length === 1 ? "platillo" : "platillos"}
              </CardTitle>
              <Button
                size="sm"
                className="gap-1.5"
