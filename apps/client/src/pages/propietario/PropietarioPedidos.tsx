...
export default function PropietarioPedidos() {
  ...
  const [filterStatus, setFilterStatus] = useState<EstadoOrden | "todos">(
    "todos",
  );

  const myRestaurants = restaurantes.filter(
    (r) => r.propietario_id === user?._id,
  );
  ...
  const getRestName = (id: string) =>
    restaurantes.find((r) => r._id === id)?.nombre || "Desconocido";
            ...
            <Card key={order._id} className="border-0 shadow-sm">
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {getRestName(order.restaurante_id)}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {new Date(order.fecha_creacion).toLocaleString("es-GT")}
                    </p>
                  </div>
                ...
                <p className="text-xs text-muted-foreground mb-3">
                  Entregar en: {order.direccion_entrega.calle},{" "}
                  ...
                  order.estado !== "cancelado" && (
                    <div className="flex gap-2">
                      <Button
                        ...
                        className="flex-1"
                        onClick={() =>
                          advanceOrderStatus(order._id, user?._id || "")
                        }
                      >
