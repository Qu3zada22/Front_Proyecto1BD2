  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-foreground">Moderacion de Resenas</h1>
      ...
      <div className="mb-4 flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Buscar en resenas..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
        </div>
      ...
      <div className="flex flex-col gap-3">
        {filtered.map((r) => (
          <Card key={r._id} className={`border-0 shadow-sm ${!r.activa ? "opacity-60" : ""}`}>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {!r.activa && <Badge variant="destructive" className="text-[10px]">Oculta</Badge>}
                  </div>
                  <p className="text-sm text-muted-foreground mb-2">{r.comentario}</p>
                  <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                    <span>Por: <span className="text-foreground">{getUserName(r.usuario_id)}</span></span>
                    <span>Restaurante: <span className="text-foreground">{getRestName(r.restaurante_id)}</span></span>
                ...
                <Button variant="ghost" size="sm" onClick={() => toggleResenaActiva(r._id)} className={r.activa ? "text-destructive" : "text-emerald-600"}>
