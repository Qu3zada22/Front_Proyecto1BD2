...
function useFetch<T>(url: string, defaultValue: T) {
  ...
  useEffect(() => {
    ...
    fetch(url)
      ...
      .then((json) => {
        if (!cancelled)
          dispatch({ type: "success", payload: json?.data ?? json });
      })
      .catch((err) => {
  ...
  return (
    <tbody>
      <tr>
        <td
          colSpan={cols}
          className="py-6 text-center text-destructive text-sm"
        >
          Error: {message}
        </td>
      </tr>
...
export default function AdminDashboard() {
  ...
  const topRest = useFetch<TopRestaurante[]>(
    "/api/reports/restaurants/top-rated?limit=10",
    [],
  );
  const topPlat = useFetch<TopPlatillo[]>(
    "/api/reports/menu-items/best-sellers?limit=10",
    [],
  );
  const ingresos = useFetch<IngresoMes[]>(
    "/api/reports/revenue/by-restaurant-month",
    [],
  );
  const topUsers = useFetch<TopUsuario[]>(
    "/api/reports/users/top-spenders?limit=10",
    [],
  );

  // Agregaciones simples
  const { data: _ordenesPorEstado } = useFetch<
    { estado: string; total: number }[]
  >("/api/reports/orders/by-status", []);
  const { data: totalOrdenesData } = useFetch<{ total: number }>(
    "/api/reports/orders/count",
    { total: 0 },
  );
  const { data: categorias } = useFetch<string[]>(
    "/api/reports/restaurants/categories/distinct",
    [],
  );
  const { data: usuariosPorRol } = useFetch<{ rol: string; total: number }[]>(
    "/api/reports/users/by-role",
    [],
  );

  const activeUsers = useMemo(
    () => usuariosPorRol.reduce((s, r) => s + r.total, 0),
    [usuariosPorRol],
  );
        ...
        <div className="flex flex-col gap-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  {topRest.loading ? (
                    ...
                    <TableError cols={5} message={topRest.error} />
                  ) : (
                    <tbody>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  {topPlat.loading ? (
                    ...
                    <TableError cols={4} message={topPlat.error} />
                  ) : (
                    <tbody>
                      {topPlat.data.map((p, i) => (
                        <tr key={i} className="border-b last:border-0">
                          ...
                          <td className="py-3 text-right font-semibold text-primary">
                            Q{Number(p.ingresos).toFixed(2)}
                          </td>
                        </tr>
                      ))}
                      {topPlat.data.length === 0 && (
                        <tr>
                          <td
                            colSpan={4}
                            className="py-6 text-center text-muted-foreground"
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  {ingresos.loading ? (
                    ...
                    <TableError cols={5} message={ingresos.error} />
                  ) : (
                    <tbody>
                      {ingresos.data.slice(0, verMasIngresos).map((row, i) => (
                        <tr key={i} className="border-b last:border-0">
                          ...
                          <td className="py-3 pr-4 text-right text-muted-foreground">
                            Q{Number(row.ticket_promedio).toFixed(2)}
                          </td>
                          <td className="py-3 text-right font-semibold text-primary">
                            Q{Number(row.total_ingresos).toFixed(2)}
                          </td>
                        </tr>
                      ))}
              ...
              {!ingresos.loading &&
                !ingresos.error &&
                ingresos.data.length > 5 && (
                  <div className="mt-4 flex items-center justify-between border-t pt-4">
                    <p className="text-xs text-muted-foreground">
                      Mostrando {Math.min(verMasIngresos, ingresos.data.length)}{" "}
                      de {ingresos.data.length} registros
                    </p>
                    <div className="flex gap-2">
                      {verMasIngresos < ingresos.data.length && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setVerMasIngresos((v) => v + 10)}
                        >
                          Ver 10 más
                        </Button>
                      )}
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  {topUsers.loading ? (
                    ...
                    <TableError cols={5} message={topUsers.error} />
                  ) : (
                    <tbody>
                      {topUsers.data.map((u, i) => (
                        <tr key={i} className="border-b last:border-0">
                          ...
                          <td className="py-3 text-right font-semibold text-primary">
                            Q{Number(u.total_gastado).toFixed(2)}
                          </td>
                        </tr>
                      ))}
