  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-foreground">Gestion de Restaurantes</h1>
        <Badge variant="secondary">{restaurantes.length} total</Badge>
      </div>

      <div className="relative mb-6">
        <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
        <Input placeholder="Buscar por nombre o categoria..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left">
              ...
              <th className="pb-3 pr-4 font-medium text-muted-foreground">Propietario</th>
              <th className="pb-3 pr-4 font-medium text-muted-foreground">Categorias</th>
              <th className="pb-3 pr-4 font-medium text-muted-foreground text-center">Calificacion</th>
              <th className="pb-3 pr-4 font-medium text-muted-foreground text-center">Resenas</th>
              <th className="pb-3 pr-4 font-medium text-muted-foreground text-center">Estado</th>
              <th className="pb-3 font-medium text-muted-foreground text-center">Accion</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((r) => (
              <tr key={r._id} className="border-b last:border-0">
                <td className="py-3 pr-4">
                  <div className="flex items-center gap-3">
                    <img src={r.img_portada} alt={r.nombre} className="h-10 w-10 rounded-lg object-cover" />
                    <div>
                      <p className="font-medium text-foreground">{r.nombre}</p>
                      <p className="text-xs text-muted-foreground">{r.direccion.calle}</p>
                    </div>
                  </div>
                </td>
                <td className="py-3 pr-4 text-foreground">{getOwnerName(r.propietario_id)}</td>
                <td className="py-3 pr-4">
                ...
                <td className="py-3 pr-4 text-center">
                  <div className="flex items-center justify-center gap-1">
                    <StarRating value={r.calificacion_prom} readOnly size={12} />
                    <span className="text-xs text-foreground">{r.calificacion_prom}</span>
                  </div>
                </td>
                <td className="py-3 pr-4 text-center text-foreground">{r.total_resenas}</td>
                <td className="py-3 pr-4 text-center">
