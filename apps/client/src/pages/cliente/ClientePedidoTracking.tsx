    ...
    <div className="mx-auto max-w-2xl px-4 py-6">
      <div className="mb-6 flex items-center gap-2">
        <Button variant="ghost" size="icon" asChild>
          <Link to="/cliente/pedidos"><ArrowLeft size={20} /></Link>
        </Button>
        <div>
          <h1 className="text-2xl font-bold text-foreground">Seguimiento de Pedido</h1>
      ...
      {orden.estado === "entregado" && (
        <Card className="mb-6 border-0 bg-emerald-50 shadow-sm">
          <CardContent className="flex flex-col items-center gap-3 p-6 text-center">
            <h3 className="text-lg font-semibold text-emerald-800">Gracias por tu preferencia!</h3>
            <p className="text-sm text-emerald-700">Tu pedido ha sido entregado exitosamente.</p>
            {!orden.tiene_resena && (
            ...
            {orden.tiene_resena && (
              <p className="text-sm text-emerald-600 font-medium">Ya dejaste una resena para este pedido</p>
            )}
          </CardContent>
        </Card>
      )}
          ...
          {orden.items.map((item, i) => (
            <div key={i} className="flex items-center justify-between border-b pb-2 last:border-0 last:pb-0">
              <div>
                ...
                <p className="text-xs text-muted-foreground">x{item.cantidad} - Q{item.precio_unitario.toFixed(2)} c/u</p>
              </div>
              <span className="font-medium text-foreground">Q{item.subtotal.toFixed(2)}</span>
            </div>
          ))}
      ...
      <Card className="mb-6 border-0 shadow-sm">
        <CardContent className="flex items-center gap-3 p-4">
          ...
          <div>
            <p className="font-medium text-foreground">{orden.direccion_entrega.alias}</p>
            <p className="text-sm text-muted-foreground">{orden.direccion_entrega.calle}, {orden.direccion_entrega.ciudad}</p>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="flex flex-col gap-3">
            {orden.historial_estados.map((h, i) => (
              <div key={i} className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium capitalize text-foreground">{h.estado.replace("_", " ")}</p>
                  <p className="text-xs text-muted-foreground">{new Date(h.timestamp).toLocaleString("es-GT")}</p>
                  {h.nota && <p className="text-xs text-muted-foreground italic">{h.nota}</p>}
                </div>
              </div>
            ))}
          </div>
      ...
      <Dialog open={showReview} onOpenChange={setShowReview}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-foreground">Dejar una Resena</DialogTitle>
          </DialogHeader>
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              <Label>Tags</Label>
              <PreferenceTags tags={REVIEW_TAGS} selected={selectedTags} onToggle={(t) => setSelectedTags((prev) => prev.includes(t) ? prev.filter((x) => x !== t) : [...prev, t])} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowReview(false)}>Cancelar</Button>
            <Button onClick={handleSubmitReview} disabled={rating === 0}>Enviar Resena</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
