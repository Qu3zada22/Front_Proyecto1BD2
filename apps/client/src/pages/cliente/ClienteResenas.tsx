...
export default function ClienteResenas() {
  ...
  const toggleAll = () => {
    setSelected((prev) =>
      prev.size === resenas.length ? new Set() : new Set(resenas.map((r) => r._id))
    )
  }
    ...
    <div className="mx-auto max-w-2xl px-4 py-6">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {re.orden_id && (
                          <> · <Link to={`/cliente/pedido/${re.orden_id}`} className="hover:underline">Ver pedido</Link></>
                        )}
                      </p>
                    </div>
                  ...
                  {re.likes?.length > 0 && (
                    <p className="mt-1 text-xs text-muted-foreground">
                      {re.likes.length} {re.likes.length === 1 ? "like" : "likes"}
                    </p>
                  )}
                </div>
              </CardContent>
      ...
      <Dialog open={!!confirmIds} onOpenChange={(open) => !open && setConfirmIds(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setConfirmIds(null)} disabled={deleting}>Cancelar</Button>
