...
export default function ClienteResenas() {
  ...
  const toggleAll = () => {
    setSelected((prev) =>
      prev.size === myResenas.length ? new Set() : new Set(myResenas.map((r) => r._id))
    )
  }
  ...
  if (myResenas.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center">
        <MessageSquare size={64} className="mx-auto mb-4 text-muted-foreground/30" />
        <h1 className="text-2xl font-bold text-foreground">No has dejado reseñas</h1>
        <p className="mt-2 text-muted-foreground">Completa un pedido y comparte tu experiencia</p>
    ...
    <div className="mx-auto max-w-2xl px-4 py-6">
      ...
      <div className="flex flex-col gap-3">
        {myResenas.map((re) => {
          ...
          const orden = re.orden_id ? ordenes.find((o) => o._id === re.orden_id) : null
            ...
            >
              <CardContent className="flex items-start gap-3 p-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {new Date(re.fecha).toLocaleDateString("es-GT", { year: "numeric", month: "long", day: "numeric" })}
                        {orden && (
                          <> · <Link to={`/cliente/pedido/${orden._id}`} className="hover:underline">Ver pedido</Link></>
                        )}
                      </p>
                    </div>
                  ...
                  {re.titulo && <p className="mt-1 font-medium text-foreground">{re.titulo}</p>}
                  {re.comentario && <p className="mt-0.5 text-sm text-muted-foreground">{re.comentario}</p>}
                  {re.tags.length > 0 && (
                  ...
                  {re.likes.length > 0 && (
                    <p className="mt-1 text-xs text-muted-foreground">{re.likes.length} {re.likes.length === 1 ? "like" : "likes"}</p>
                  )}
                </div>
              </CardContent>
            </Card>
      ...
      {/* Confirm dialog */}
      <Dialog open={!!confirmIds} onOpenChange={(open) => !open && setConfirmIds(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-foreground">
              {confirmIds?.length === 1 ? "Eliminar reseña" : `Eliminar ${confirmIds?.length} reseñas`}
            </DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setConfirmIds(null)}>Cancelar</Button>
