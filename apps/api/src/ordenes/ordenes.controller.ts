...
export class OrdenesController {
  ...
  @ApiOperation({
    ...
    description:
      'Crea una orden dentro de una **transacción ACID** (replica set requerido). El total se calcula automáticamente.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Filtra por cliente, restaurante o estado. Usa índice compuesto.',
  })
  @ApiQuery({
    name: 'cliente_id',
    required: false,
  ...
  @ApiOperation({
    ...
    description: 'Incluye populate de cliente y restaurante.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Estados válidos: pendiente → en_proceso → en_camino → entregado | cancelado.',
  })
  @ApiParam({ name: 'id', description: 'ObjectId de la orden' })
  @ApiBody({
    schema: {
      example: {
        estado: 'en_proceso',
        actor_id: '64a1b2c3d4e5f6a7b8c9d0e1',
        nota: 'Cocina confirmó la orden',
      },
    },
  })
  updateStatus(
  ...
  @ApiOperation({
    ...
    description: 'deleteMany por array de IDs.',
  })
  @ApiBody({
    schema: {
      example: {
