...
export class CreateOrdenDto {
    @ApiProperty({ example: '64a1b2c3d4e5f6a7b8c9d0e1', description: 'ID del cliente (usuario)' })
    ...
    @Type(() => ItemOrdenDto) items: ItemOrdenDto[];

    @ApiProperty({ type: DireccionEntregaDto })
    @ValidateNested() @Type(() => DireccionEntregaDto) direccion_entrega: DireccionEntregaDto;
