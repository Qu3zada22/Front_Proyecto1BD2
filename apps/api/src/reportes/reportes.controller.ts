...
export class ReportesController {
  ...
  @ApiOperation({
    ...
    description: 'Aggregation simple: $group por estado.',
  })
  ordenesPorEstado() {
  ...
  @Get('orders/count')
  @ApiOperation({
    summary: 'Total de órdenes',
    description: 'Aggregation con $count.',
  })
  totalOrdenes() {
  ...
  @ApiOperation({
    summary: 'Categorías distintas',
    description: 'Usa distinct() sobre el campo categorias (índice multikey).',
  })
  categoriasDistintas() {
  ...
  @ApiOperation({
    ...
    description: 'Aggregation simple: $group por rol.',
  })
  usuariosPorRol() {
  ...
  @ApiOperation({
    summary: 'Top restaurantes',
    description:
      'Aggregation compleja: parte de resenas (fuente de verdad) → $group avg/count → $match(≥5 reseñas) → $sort/$limit → $lookup restaurantes → $project.',
  })
  ...
  @ApiOperation({
    summary: 'Platillos más vendidos',
    description:
      'Aggregation: $unwind items + $group por item_id + $sort por cantidad.',
  })
  @ApiQuery({ name: 'limit', required: false, type: Number, example: 10 })
  ...
  @Get('revenue/by-day')
  @ApiOperation({
    summary: 'Ingresos por día',
    description:
      'Aggregation: $match por rango de fechas + $group con $dateToString + suma de totales.',
  })
  @ApiQuery({
    name: 'desde',
    required: false,
    example: '2023-01-01',
    description:
      'Fecha inicio ISO (default: 2023-01-01, datos entre 2023-2025)',
  })
  @ApiQuery({
    name: 'hasta',
    required: false,
    example: '2025-12-31',
    description: 'Fecha fin ISO (default: 2025-12-31)',
  })
  ...
  @ApiOperation({
    summary: 'Restaurantes por categoría',
    description: 'Aggregation: $unwind categorias + $group por categoría.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Aggregation: $group por restaurante_id + año + mes → $lookup restaurante. Muestra el mes más reciente primero.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Aggregation: $match entregado → $group por usuario_id con $sum total → $sort → $lookup usuario (sin password).',
  })
  @ApiQuery({ name: 'limit', required: false, type: Number, example: 10 })
  usuariosConMayorGasto(@Query('limit') limit: string) {
    return this.reportesService.usuariosConMayorGasto(
      Math.max(1, +limit || 10),
    );
  }
}
