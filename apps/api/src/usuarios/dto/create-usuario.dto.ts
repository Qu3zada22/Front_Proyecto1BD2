...
export class DireccionUsuarioDto {
    ...
    @ApiPropertyOptional({ example: true }) @IsOptional() @IsBoolean() es_principal?: boolean;

    @ApiPropertyOptional({ type: CoordsDto })
    @IsOptional() @ValidateNested() @Type(() => CoordsDto) coords?: CoordsDto;
}
...
export class CreateUsuarioDto {
    ...
    @ApiPropertyOptional({ enum: ['cliente', 'propietario', 'admin'], example: 'cliente' })
    ...
    @Type(() => DireccionUsuarioDto) direcciones?: DireccionUsuarioDto[];
}
