...
export class RestaurantesController {
  ...
  @ApiOperation({
    ...
    description: 'Filtra por categoría, búsqueda de texto o estado activo.',
  })
  @ApiQuery({ name: 'categoria', required: false })
  @ApiQuery({
    name: 'busqueda',
    required: false,
    description: 'Búsqueda full-text (usa índice text)',
  })
  ...
  @Get('near')
  @ApiOperation({
    summary: 'Restaurantes cercanos',
    description: 'Usa índice 2dsphere ($near) para búsqueda geoespacial.',
  })
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'Distancia en metros (default 5000)',
  })
  findNear(
    ...
    if (isNaN(lngNum) || isNaN(latNum)) {
      throw new BadRequestException(
        'lng y lat son requeridos y deben ser números válidos',
      );
    }
    return this.restaurantesService.findNear(
      lngNum,
      latNum,
      +maxDistance || 5000,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Transacción ACID: marca restaurante activo=false, platillos disponible=false y cancela todas las órdenes activas del restaurante.',
  })
  @ApiParam({ name: 'id', description: 'ObjectId del restaurante' })
