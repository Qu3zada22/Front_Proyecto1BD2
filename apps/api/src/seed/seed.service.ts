...
export class SeedService {
    ...
    async run(): Promise<{ message: string; counts: Record<string, number> }> {
        ...
        const [usuarios, restaurantes, menuItems, ordenes, resenas] = await Promise.all([
    ...
    private runIngestScript(): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            const ingestPath = path.join(monoRepoRoot, 'apps', 'database', 'ingest.js');
            ...
            child.on('close', (code) => {
                if (code === 0) resolve();
                else reject(new InternalServerErrorException(`Ingest script exited with code ${code}`));
            });
        });
    }
