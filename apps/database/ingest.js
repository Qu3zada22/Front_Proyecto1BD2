...
const COLLECTIONS  = ['usuarios', 'restaurantes', 'menu_items', 'ordenes', 'resenas']
...
async function main() {
  ...
  const ownersFromDB = await db.collection('usuarios')
  ...
  const clientsFromDB = await db.collection('usuarios')
    .find({ rol: 'cliente' })
    .toArray()
  ...
  for (const batch of generateOrderBatches(clientsFromDB, restaurantMenuData, TOTAL_ORDERS, BATCH_SIZE)) {
  ...
  const { modifiedCount: marcadas } = await db.collection('ordenes').updateMany(
    { _id: { $in: reviewedIds } },
  ...
  const ratings = await db.collection('resenas').aggregate([
    { $match: { activa: true } },
    { $group: { _id: '$restaurante_id', prom: { $avg: '$calificacion' }, total: { $sum: 1 } } },
  ]).toArray()
  ...
  await db.collection('restaurantes').bulkWrite(
    ratings.map((r) => ({
      updateOne: {
        filter: { _id: r._id },
        update: { $set: { calificacion_prom: Math.round(r.prom * 10) / 10, total_resenas: r.total } },
      },
    })),
    { ordered: false }
  )
  ...
  for (const rest of restsFromDB) {
    const fileId = await uploadBuffer(db, png, `portada_${rest._id.toHexString()}.png`, {
      type: 'restaurant_cover', restaurante_id: rest._id,
    })
    coverOps.push({
      updateOne: { filter: { _id: rest._id }, update: { $set: { img_portada_id: fileId } } },
    })
  }
  ...
  for (const item of menuFromDB.filter((_, i) => i % 9 < 3)) {
    const fileId = await uploadBuffer(db, png, `item_${item._id.toHexString()}.png`, {
      type: 'menu_item', item_id: item._id,
    })
    imageOps.push({
      updateOne: { filter: { _id: item._id }, update: { $set: { imagen_id: fileId } } },
    })
  }
