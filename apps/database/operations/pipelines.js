    ...
    .aggregate([
      ...
      { $unwind: "$items" },
      {
        $group: {
          ...
          precio: {
            $first: { $ifNull: ["$items.precio", "$items.precio_unitario"] },
          },
...
export async function pushDireccion(db, userId, direccion) {
  return db
    .collection("usuarios")
    .updateOne({ _id: userId }, { $push: { direcciones: direccion } });
}
...
export async function pullDireccion(db, userId, alias) {
  return db
    .collection("usuarios")
    .updateOne({ _id: userId }, { $pull: { direcciones: { alias } } });
}
...
export async function addSetPreferencia(db, userId, preferencia) {
  return db
    .collection("usuarios")
    .updateOne({ _id: userId }, { $addToSet: { preferencias: preferencia } });
}
...
export async function addSetLike(db, resenaId, userId) {
  return db
    .collection("resenas")
    .updateOne({ _id: resenaId }, { $addToSet: { likes: userId } });
}
...
export async function pullLike(db, resenaId, userId) {
  return db
    .collection("resenas")
    .updateOne({ _id: resenaId }, { $pull: { likes: userId } });
}
