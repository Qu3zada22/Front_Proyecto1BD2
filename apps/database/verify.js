...
async function main() {
  ...
  const sampleRest = await db.collection('restaurantes').findOne({ nombre: 'El Portal Chapín' })
  const sampleUser = await db.collection('usuarios').findOne({ email: 'ana.garcia@email.com' })
  ...
  for (const col of ['usuarios', 'restaurantes', 'menu_items', 'ordenes', 'resenas']) {
    const n = await P.count(db, col)
    console.log(`  ${col.padEnd(15)} → ${n.toLocaleString()}`)
  }

  section('COUNT: órdenes por estado')
  for (const estado of ['pendiente', 'en_proceso', 'en_camino', 'entregado', 'cancelado']) {
  ...
  console.log(' ', await P.distinct(db, 'menu_items', 'etiquetas', { disponible: true }))
  ...
  {
    const r = await db.collection('usuarios')
  ...
  {
    const r = await db.collection('restaurantes')
      .find({
        ubicacion: {
          $near: { $geometry: { type: 'Point', coordinates: [-90.5064, 14.6048] }, $maxDistance: 5000 },
        },
      })
      .explain('executionStats')
    const s = r.executionStats
  ...
  section('EXPLAIN — índice compuesto ESR (restaurante_id + estado + fecha_creacion)')
  {
    const r = await db.collection('ordenes')
  ...
  {
    const r = await db.collection('resenas')
  ...
  for (const [i, r] of (await P.topMenuItems(db, 10)).entries()) {
    console.log(`  ${String(i + 1).padStart(2)}. ${r.item.padEnd(30)} ×${r.veces_ordenado}  Q${r.precio}  — ${r.restaurante}`)
  }

  section('PIPELINE P3 — Restaurantes con rating real (desde reseñas)')
  for (const r of await P.restaurantesConRating(db)) {
    console.log(`  ${r.nombre.padEnd(28)} ★ ${String(r.rating_real).padStart(3)}  (${r.resenas_activas} reseñas activas)`)
  }

  section('PIPELINE P4 — Distribución de calificaciones por restaurante')
  for (const r of await P.distribucionCalificaciones(db)) {
    const breakdown = r.breakdown.sort((a, b) => a.calificacion - b.calificacion)
      .map((b) => `${b.calificacion}★:${b.cantidad}`).join('  ')
  ...
  for (const r of await P.topResenasPorLikes(db, 5)) {
    console.log(`  "${r.titulo}"  ★${r.calificacion}  👍${r.num_likes}  — ${r.autor} en ${r.restaurante}`)
  }
  ...
  section('$push — agregar dirección embebida a usuario')
  {
    const res = await P.pushDireccion(db, sampleUser._id, {
      alias: 'Gym', calle: '6a Calle 4-10 Zona 10',
      ciudad: 'Guatemala City', pais: 'Guatemala', es_principal: false,
    })
    console.log(`  modifiedCount: ${res.modifiedCount}`)
    const u = await db.collection('usuarios').findOne({ _id: sampleUser._id }, { projection: { 'direcciones.alias': 1 } })
    console.log(`  direcciones ahora: ${u.direcciones.map((d) => d.alias).join(', ')}`)
  }
  ...
  {
    const userCarlos = await db.collection('usuarios').findOne({ email: 'carlos.lopez@email.com' })
    ...
    const u = await db.collection('usuarios').findOne({ _id: userCarlos._id }, { projection: { preferencias: 1 } })
    console.log(`  preferencias: ${JSON.stringify(u.preferencias)}  (saludable aparece 1 sola vez)`)
  }
  ...
  section('$addToSet + $pull — likes en reseña')
  {
    ...
    if (resena) {
      ...
      const after = await db.collection('resenas').findOne({ _id: resena._id }, { projection: { likes: 1 } })
      console.log(`  likes después de 2 addToSet: ${after.likes.length}`)
      await P.pullLike(db, resena._id, sampleUser._id)
      const final = await db.collection('resenas').findOne({ _id: resena._id }, { projection: { likes: 1 } })
  ...
  files.slice(0, 5).forEach((f) => console.log(`  - ${f.filename}  (${f.length} bytes)`))
