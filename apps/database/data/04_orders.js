...
const PESOS = [0.05, 0.1, 0.1, 0.55, 0.2];
 ...
 */
export function* generateOrderBatches(
  clientDocs,
  restaurantMenuData,
  total = 50_000,
  batchSize = 2_000,
  ...
  while (generated < total) {
    ...
    const addr =
      client.direcciones.find((d) => d.es_principal) ?? client.direcciones[0];
    ...
    const numItems = pickWeighted([1, 2, 3], [0.35, 0.45, 0.2]);
    const chosen = [...restMeta.items]
    ...
    const totalNum = orderItems.reduce(
      (s, i) => s + parseFloat(i.precio_unitario.toString()) * i.cantidad,
      0,
    );
    ...
    const doc = {
      ...
      items: orderItems,
      estado,
      historial_estados: buildHistorial(
        estado,
        fechaCreacion,
        client._id,
        restMeta.propietario_id,
      ),
      total: dec(totalNum),
      ...
      fecha_entrega_real:
        estado === "entregado"
          ? new Date(
              fechaCreacion.getTime() + (35 + Math.random() * 40) * 60 * 1000,
            )
          : null,
      tiene_resena: false,
    };
