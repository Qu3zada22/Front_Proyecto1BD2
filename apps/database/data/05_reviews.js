...
const ALL_TAGS = ['rapido', 'sabroso', 'limpio', 'ambiente', 'precio', 'atencion']
...
function pickTags(calificacion) {
  const pool = calificacion >= 4 ? TAGS_POS : calificacion === 3 ? TAGS_NEU : TAGS_NEG
...
export function generateReviews(deliveredOrders, clientIds = []) {
  ...
  for (const order of deliveredOrders) {
    ...
    const numLikes = calificacion >= 4 ? Math.floor(Math.random() * 8) : Math.floor(Math.random() * 3)
